  ...
  $('button.btn-active-user').click(function () {
    ...
    $.ajax({
      ...
      success: function success(response) {
        ...
        showMessage('success', response.user.first_name + ' ' + response.user.last_name + ' was active successfully!'); // Change disabled

        $(trObj).find('button.btn-inactive-user').removeAttr('disabled');
        $(trObj).find('button.btn-active-user').attr('disabled', true); // Change active content
  ...
  $('button.btn-inactive-user').click(function () {
    ...
    $.ajax({
      ...
      success: function success(response) {
        ...
        showMessage('success', response.user.first_name + ' ' + response.user.last_name + ' was inactive successfully!'); // Change disabled

        $(trObj).find('button.btn-active-user').removeAttr('disabled');
        $(trObj).find('button.btn-inactive-user').attr('disabled', true); // Change active content
  ...
  $('button#btn-delete-account').click(function () {
    ...
    $.ajax({
      ...
      success: function success(res) {
        ...
        if (res.type == 'success') {
          // Show message
          showMessage('success', res.user.first_name + ' ' + res.user.last_name + ' was deleted successfully!'); // Remove tr
          // $(trObj).remove();
          // Change role content

          $(trObj).find('td:nth-child(3)').removeClass('text-success');
          $(trObj).find('td:nth-child(3)').addClass('text-danger');
          $(trObj).find('td:nth-child(3)').text(res.user.role_name); // Change active content

          $(trObj).find('td:nth-child(4)').removeClass('text-success');
          $(trObj).find('td:nth-child(4)').addClass('text-danger');
          $(trObj).find('td:nth-child(4)').text(res.user.active_name); // Change tr attribute

          $(trObj).attr('data-userrole', res.user.role);
          $(trObj).attr('data-useractive', res.user.active); // Change disabled
  ...
  $('button#btn-update-user').click(function () {
    ...
    $.ajax({
      ...
      success: function success(res) {
        ...
        if (res.type == 'success') {
          // Show message
          showMessage('success', res.user.first_name + ' ' + res.user.last_name + ' was updated successfully!'); // Remove tr
          // $(trObj).remove();
          // Change role content
          ...
          $(trObj).attr('data-userrole', res.user.role);
          $(trObj).attr('data-useractive', res.user.active); // Change disabled delete button
